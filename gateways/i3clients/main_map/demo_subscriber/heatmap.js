/********************************INITIALIZE MAP*************************************************/
	...
	for(i=0;i<serviceRequests.length;i++){
	    heatmapData.push(new google.maps.LatLng(serviceRequests[i].lat,serviceRequests[i].lng));
	}/*
	...
	setTimeout(function(){setMarkers();},5000);
	//setTimeout(setCr,5000);
    }


    function initHeatmap(requestString){
	document.getElementById("heading").innerHTML = requestString;
	...
	var heatmapB = [];
	for(i=0;i<serviceRequests.length;i++){
	  if(serviceRequests[i].requesttype == requestString){
	    heatmapB.push(new google.maps.LatLng(serviceRequests[i].lat,serviceRequests[i].lng));
	  }
	}
	var heatmap = new google.maps.visualization.HeatmapLayer({
  	  data: heatmapB
	...
	var mapNames = ['Bulky Items','Dead Animal Pickup','Electronic Waste','Graffiti Removal','Homeless Encampment',
				'Illegal Dumping Pickup','Metal/Household Appliances','Multiple Streetlight Issues',
				'Report Water Waste','Single Streetlight Issue']
	var i=0;
	var showHM = setInterval(function(){
					if(i<mapNames.length){
						initHeatmap(mapNames[i]);i++}
					else clearInterval(showHM);},5000);
    }
...
/*********************************SET MARKER FUNCTIONS*******************************************************/
    ...
    function setMarkers(){
	setTimeout(function(){showHeatmaps();},3000);
	//setAqi();
	//setTrafficInc();
	//var sr = setServReq();
	...
	var setSR = setInterval(function(){
					    ...
					    else {clearInterval(setSR); setTimeout(function(){showHeatmaps();},8000); }
    ...
    function setMarker(i){
      ...
      //markers.servReq[i].marker.setIcon('ss.png');
